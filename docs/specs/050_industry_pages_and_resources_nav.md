...
**Status:** Scaffold built (industry page bodies pending copy).
...
**Reference:** Industry pages follow the existing service-page pattern (spec 037) — a slug registry rendered through a shared route. The Resources dropdown is the menu originally planned in spec 034 (the nav UI already anticipated it).
...
- **Scaffold pages only.** Each page renders the shared nav + footer (via the `(inner)` layout) and a placeholder `CenteredHero` (eyebrow + industry name). The industry-specific positioning/messaging body is **deferred** to a follow-up — no marketing copy is authored in this pass.
...
- **Industry positioning/messaging copy and full page bodies** — a deferred follow-up. This pass only stands up routing, metadata, and nav.
- **A dedicated `IndustryPageTemplate`** — not built yet; pages currently render a placeholder hero. The template is introduced when the page bodies are authored.
...
- [ ] `/industries/health-wellness`, `/industries/home-services`, and `/industries/small-business` each render the shared nav + footer with a placeholder hero; an unknown industry slug 404s.
- [ ] Each industry page has a per-industry `<title>` / meta description.
- [ ] The top-level menu reads **Solutions**, and its mega menu shows a "By industry" column linking to the three industry pages.
- [ ] The Blog top-level item is replaced by a **Resources** dropdown containing Blog and Grader; the Grader leaf and Grader promo open in a new tab.
- [ ] Desktop and mobile nav both reflect the above.
