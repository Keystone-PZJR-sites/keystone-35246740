# keystone design tokens — architecture and re-extraction

Variables, styles, and semantic layers of the Figma file `ks-MarketingSite`
(IBO39siJYDhiCRtuLWUTW2), as extracted through the console bridge on
**2026-08-22**. The durable parts are the architecture (§0), the naming
conventions, the implementation notes (§8), and the re-extraction procedure
(§9). The value tables are a snapshot: the built values live in
`design-system/v2/tokens/` and the code is the source of truth for them.

**Figma is the live source of truth — re-extract before building** (§9 has the
snippets). The site ships light mode only (owner decision); the dark-mode
columns document the library, not a shipped theme. Companion:
`grid-engine.md` (layout).
...
`+` sizes are half-steps inserted for the responsive anchors.
...
Relationship to the grid (`grid-engine.md`): the 16px **cell** is `spacing-xl`; the fluid **tick** is not a token (it's `100cqw/12`). Don't tokenize tick.
...
- Emit three CSS layers mirroring Figma: `primitives` → `semantic` → `component`. Light mode only ships; no dark-mode remapping is emitted.
