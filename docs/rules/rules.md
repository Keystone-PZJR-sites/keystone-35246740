...
- **Audits at rest.** Mid-flight motion may be transiently fractional-tick by design; the stack-sum and landmark audits are asserted at rest in every state. Know the harness gap: the sweep audits the vertical stack only — horizontal clearance against exposed cells is not asserted, and the footer's rail collision hid there until the 011 review (2026-08-27). Verify clearance manually at compressed-slice widths until the page expectations gain the assertion (flagged for spec 013).
