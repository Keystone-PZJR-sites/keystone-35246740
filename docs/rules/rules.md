...
- Every spec ships a **permanent, noindexed dev route** as its QA surface (`/grid`, `/primitives`, `/footer`, `/nav`, and the section routes — since the 023 cutover the homepage's are `/hero-next` · `/system-next` · `/engines-free-2` · `/work-next` · `/case-carousel-next` (023 §9 R2: the standing names are permanent); `/home-fixture` is the QA mount of the assembled homepage `/` renders).
...
`/` renders the assembled homepage (the v2 composition since the spec 023 cutover; the v1 homepage and its routes retired with it). The dev routes (`/grid`, `/primitives`, `/footer`, `/nav`, `/hero-next`, `/system-next`, `/engines-free-2`, `/work-next`, `/case-carousel-next`, `/home-fixture`, and the other page fixtures) are permanent QA surfaces and noindexed. The nav links to pages that do not exist yet 404 by owner decision (spec 010 §7 F1) until those pages are built.
