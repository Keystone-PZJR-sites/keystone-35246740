...
| Our Work | 014 (header + case studies, **built 2026-08-28**) · 015 (gallery) · 016 (overlay + assembly) | **in build** — **014 approved + built + verified 2026-08-28** (zero-drift re-extraction; every landmark rendered-bounds-re-verified at build; two §2 deviations + the label-wrap erratum amended in place — 014 §9 build record; verified at the five anchors + nine slice widths; `/our-work` + `/our-work-fixture` live on the interim composition, footer at the section end until 015). Same-evening owner revisions (014 §4/§6.0): the site-image stroke and the **rises-only entrance** (no nav beat, no sweep, no highlights; the 006 orchestrator generalized at its second consumer — the page's one island; G8 audits this choreography with the rest). 015 next; overlay inputs (planning F3) arrive after the sections are built, needed before 016; the page expectations + sweep leg land with 016 |
| Case Studies | — | queued directly after the Our Work phase (owner 2026-08-28 — the design is ready); the 014 View-Case-Study CTAs target its `/case-studies/{slug}` routes and 404 until it lands |
