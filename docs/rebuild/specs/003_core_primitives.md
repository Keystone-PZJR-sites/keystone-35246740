...
**Sources:** fresh MCP reads 2026-08-22 of the component sets
`button-fill` (486:5251) · `button-ghost` (90:8297) · `grader-input`
(503:25844) · `icons` (519:5431), all in `ks-MarketingSite`. Design
decisions recorded 2026-08-22: ghost focus is derived (below); fill
buttons never disable; grader error/loading states are deferred (design
will share nodes; they are specced with the section that wires
submission).
...
## 4 · grader-input (503:25844)

Variants: size `lg/md/sm` × state `default/hover/focus/active/filled`.
White pill (`bg-000`, 1px border, `--radius-full`) · 20px sparkle icon ·
placeholder text · circular submit button. A 4px ring is **always
present** and recolors per state; heights are 66 / 50 / 44.

| size | padding | gap | text style | submit circle | arrow |
|---|---|---|---|---|---|
| lg | 16 · 12 (y 12) | 16 | `text/xl/Light` | 40 | 24 |
| md | 12 · 8 (y 8) | 16 | `text/md/Light` | 32 | 16 |
| sm | 12 · 8 (y 8) | 16 | `text/md/Light` | 28 | 14 |

| state | border | ring | text ink | submit bg |
|---|---|---|---|---|
| default | `border-000` | `bg-200` | `text-500` | `teal-300` |
| hover | `teal-250` | `teal-200` | `text-400` | `teal-300` |
| focus | `teal-400` | `teal-300` | `text-400` | `teal-300` |
| active | `teal-300` | `teal-200` | `text-200` | `teal-300` |
| filled | `teal-300` | `teal-200` | `text-200` | `teal-400` |
...
the value is non-empty). Error and loading are **out of scope** here and
arrive with the section that wires submission.

## 5 · Icons (519:5431)

Five 20-grid outline icons: `projects` · `approach` · `case-studies` ·
`sparkle` · `chat`. Exported from Figma and committed (never
hand-authored); the only permitted edit is normalizing paint attributes
to `currentColor` — geometry is never touched. Component-embedded glyphs
(the fill button's 10px nav-trigger, the grader's submit arrow) ship the
same way, inside their components. All decorative icons are hidden from
the accessibility tree; the owning control carries the name.
...
three components, the icon set, and the token catalog — the visual-QA
surface spec 001's last criterion calls for.

## 8 · Acceptance criteria

- [ ] Every variant × state in the three sets renders pixel-identical to
