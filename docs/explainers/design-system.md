...
| Components | `design-system/components/` | Composite, reusable chrome: the site nav (`HeroNav`, used on every page), footer, lead-capture modal, `CtaModalButton` (the one button that opens the lead-capture "get a free demo" modal), and `InnerPageShell`; the shared card toolkit (`SpotlightCard`, `CardGrid`); `Marquee` — the seamless infinite-scroll strip behind `LogoMarquee` and the Our Story photo strip; and `Pagination` — the reusable Previous / page numbers / Next control. |
...
| Patterns | `design-system/patterns/` | Page-specific component groups that are not site-wide chrome (`blog/`, `legal/`, `services/`, `industries/` — the enriched "by business type" landing template (audience tags, benefits, old-vs-Keystone contrast, capabilities, how-it-works steps, results, proof, what's-included, FAQ) — `case-studies/` — the customer-story gallery card and the block-driven long-form detail template (TL;DR, prose, callouts, charts, stat bands, comparison, tools, timeline, quote, gallery) — `gallery/` — the live-site showcase entry + fullscreen iframe takeover with mobile/tablet/full viewport switcher, plus an optional case-study results strip on entries backed by a case study (spec 054) — `pricing/` — two-plan checkout cards wired to Stripe Payment Links (spec 055) — `how-it-works/` — the tokenized `ProcessMock` step visuals — and `careers/` — the decorative `TeamCollage` hero collage). Not re-exported from the top barrel — import directly. |
