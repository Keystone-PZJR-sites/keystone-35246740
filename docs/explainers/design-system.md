...
| Patterns | `design-system/patterns/` | Page-specific component groups that are not site-wide chrome (`blog/`, `legal/`, `services/`, `how-it-works/` — the tokenized `ProcessMock` step visuals — and `careers/` — the decorative `TeamCollage` hero collage). Not re-exported from the top barrel — import directly. |
