...
| Patterns | `design-system/patterns/` | Page-specific component groups that are not site-wide chrome (`blog/`, `legal/`, `services/`, `industries/`, `case-studies/` — the customer-story gallery card and the block-driven long-form detail template (TL;DR, prose, callouts, charts, stat bands, comparison, tools, timeline, quote, gallery) — `how-it-works/` — the tokenized `ProcessMock` step visuals — and `careers/` — the decorative `TeamCollage` hero collage). Not re-exported from the top barrel — import directly. |
