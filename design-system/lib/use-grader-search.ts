...
/** Grader typeahead state. Debouncing, cancellation, grouped limits, and
...
type GraderSearchStatus = "idle" | "loading" | "ready";
...
export function useGraderSearch(query: string, enabled = true): GraderSearchState {
  ...
  return useMemo<GraderSearchState>(() => {
    ...
    if (answer.resultsFor === q) return state("ready", answer.groups, q);
    return state("loading", EMPTY_GROUPS, "");
  }, [q, answer]);
}
