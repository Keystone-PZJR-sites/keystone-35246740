/** The grader input asks the Grader API for business
 ...
 * prerender rather than shipping a guessed endpoint. */
...
const GRADER_API_BASE_URL = trimTrailingSlash(
...
const GRADER_SEARCH_PATHS = splitCsv(
...
const GRADER_SEARCH_TIMEOUT_MS = 8000;
...
const EMPTY_GRADER_SEARCH: GraderSearchResponse = { places: [], web: [] };
...
/** Drop malformed entries from the grouped API response. */
function normalizeGraderSearchResponse(body: unknown): GraderSearchResponse {
...
function graderSearchApiUrl(query: string, path: string): string {
