...
export interface MobileEveryChannelPillData {
  label: string;
  color: string;
  /** Dot color — defaults to the hero-text token if omitted. */
  dotColor?: string;
...
export interface MobileEveryChannelProps {
  ...
  /** Section background color. Defaults to the hero-surface token. */
  bgColor?: string;
}
...
/**
 * Mobile-only Every Channel section (below 985px).
 *
...
export function MobileEveryChannel({
  ...
  videoSrcs,
  pills,
  bgColor = 'var(--color-hero-surface,#063126)',
        ...
        {sortedPills.map((pill, i) => (
          ...
          >
            <div
              ...
              style={{ backgroundColor: pill.dotColor ?? 'var(--color-hero-text,#f0eee6)' }}
            />
            <span
              className="mec-pill-label"
              style={{ color: pill.dotColor ?? 'var(--color-hero-text,#f0eee6)' }}
            >
              {pill.label}
            </span>
          </div>
