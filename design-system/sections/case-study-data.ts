/** Typed content and intrinsic image dimensions for published case studies.
 * Only records in `CASE_STUDIES` produce routes. */
...
interface FunnelRow {
  label: string;
  value: string;
  /** `"fill"` consumes the remaining row width; numbers are pixels. */
  bar: "fill" | number;
}
...
export interface StackCell {
  ...
  title: string;
  desc: string;
  /** Optional compact description for the narrow bands. */
  descShort?: string;
}

interface ImageTierSize {
  w: number;
  h: number;
}
...
export interface CaseStudy {
  ...
  assetId: string;
  name: string;
  /** Segments support responsive visual breaks while preserving one heading. */
  h1: { seg1: string; seg2: string; seg3: string };
  metadata: { category: string; location: string; founders: string; since: string };
  /** Canonical tag order plus the alternate tablet order. */
  tags: CaseStudyTag[];
  tagsRt: CaseStudyTag[];
  intro: { head: string; stats: CaseStudyStat[]; disclaimer: string };
  overview: { head: string; body: string; items: string[]; stats: CaseStudyStat[] };
  ...
  shift: {
    ...
    /** Each entry renders as one list row. */
    beforeLines: string[];
    afterItems: string[];
    tag: string;
  };
...
const YOUR_HEALTH_SOLUTIONS: CaseStudy = {
  slug: "your-health-solutions",
  assetId: "yhs",
  name: "Your Health Solutions",
  /* Empty trailing segments let this heading wrap naturally. */
  h1: {
...
const BARE_LUX_STUDIO: CaseStudy = {
...
/** Published studies in route order. Unknown slugs return no record. */
