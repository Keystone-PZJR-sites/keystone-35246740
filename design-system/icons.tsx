...
/** icons/sparkle (91:8477 teal · 519:5462 red · 921:18505 brown).
 * Byte-identical geometry uses the --sparkle-a/b palette hooks. */
...
/** Slider arrow (613:21217). Its distinct 13:10 cut keeps an intrinsic
 * height derived from the requested width. */
...
/** Case-study stat star. `size` is width; height follows its 15:14 ratio. */
...
/** Footer chevron (523:19955) — 16-grid stroke in currentColor. */
...
/** FAQ drawer chevron with fixed stroke geometry. */
...
/** Mobile nav menu (520:15982) — 28-grid, currentColor. */
...
/** icons/grader (540:23964) — 20-grid, currentColor. */
...
/** icons/podcast (540:23963) — 20-grid, currentColor. */
...
/** icons/blog (540:23965) — 20-grid, currentColor. */
...
/** icons/website (613:21309 gray · 1038:15845 teal).
 * Byte-identical geometry uses the --website-a/b palette hooks
 * (a: frame and dots · b: sparkle). */
export function IconWebsite({ size = 20, className }: Props) {
  return (
    <svg width={size} height={size} viewBox="0 0 20 20" fill="none" aria-hidden="true" className={className} xmlns="http://www.w3.org/2000/svg">
      <path d="M5.41667 6.25033C5.41667 6.71056 5.04357 7.08366 4.58333 7.08366C4.1231 7.08366 3.75 6.71056 3.75 6.25033C3.75 5.79009 4.1231 5.41699 4.58333 5.41699C5.04357 5.41699 5.41667 5.79009 5.41667 6.25033Z" fill="var(--website-a, var(--color-lightgray-800, #b1aa9a))" />
      <path d="M7.91667 6.25033C7.91667 6.71056 7.54357 7.08366 7.08333 7.08366C6.6231 7.08366 6.25 6.71056 6.25 6.25033C6.25 5.79009 6.6231 5.41699 7.08333 5.41699C7.54357 5.41699 7.91667 5.79009 7.91667 6.25033Z" fill="var(--website-a, var(--color-lightgray-800, #b1aa9a))" />
      <path d="M9.58333 7.08366C10.0436 7.08366 10.4167 6.71056 10.4167 6.25033C10.4167 5.79009 10.0436 5.41699 9.58333 5.41699C9.12308 5.41699 8.75 5.79009 8.75 6.25033C8.75 6.71056 9.12308 7.08366 9.58333 7.08366Z" fill="var(--website-a, var(--color-lightgray-800, #b1aa9a))" />
      <path d="M1.25 2.91699V16.2503H9.75V15.0003H2.5V4.16699H16.6667V8.08366H17.9167V2.91699H1.25Z" fill="var(--website-a, var(--color-lightgray-800, #b1aa9a))" />
      <path d="M11.7506 14.1087C12.9019 14.1087 13.6109 14.3635 14.0409 14.7934C14.4708 15.2234 14.7256 15.9325 14.7256 17.0837H15.7756C15.7756 15.9325 16.0304 15.2234 16.4604 14.7934C16.8904 14.3635 17.5994 14.1087 18.7507 14.1087V13.0587C17.5994 13.0587 16.8904 12.8039 16.4604 12.3739C16.0304 11.9439 15.7756 11.2349 15.7756 10.0837H14.7256C14.7256 11.2349 14.4708 11.9439 14.0409 12.3739C13.6109 12.8039 12.9019 13.0587 11.7506 13.0587V14.1087Z" fill="var(--website-b, var(--color-darkgray-100, #847f71))" />
    </svg>
  );
}
...
/* Gallery rail icons (661:9454–9458 · 661:9471). Flattened fills scale
 * from the 14-grid source; currentColor lets button state supply ink. */
...
/* Case-study stack glyphs use hue-toned two-color cuts at their intrinsic
 * boxes. They are distinct from the gray list-icon family. */

/** Double checkmark — intrinsic teal/400 and teal/500 outlined fills. */
...
/** Intro-stat star. Its distinct 25:24 cut derives height from width. */
...
/** stack/website — teal two-tone, 32-grid. */
...
/** stack/meta-ads — orange two-tone, 28-grid. */
...
/** stack/front-desk — blue two-tone, 32-grid. */
...
/** stack/content — yellow two-tone, 28-grid. */
...
/** stack/reviews — pink two-tone, 26-grid. */
...
/** stack/reporting — purple two-tone, 30-grid. */
...
/** icons/system-intersect (799:60946). Its rendered-bounds viewBox scales
 * proportionally with the system ring. */
...
/** Loading glyph with currentColor head and overridable --loading-track. */
...
/** Spotify mark; its mount supplies the active color. */
export function IconSocialSpotify({ size = 34, className }: Props) {
  return (
    <svg width={size} height={size} viewBox="0 0 34 34" fill="none" aria-hidden="true" className={className} xmlns="http://www.w3.org/2000/svg">
      <path d="M16.8011 2.7998C9.06907 2.7998 2.80078 9.06793 2.80078 16.7999C2.80078 24.5321 9.06907 30.7998 16.8011 30.7998C24.5338 30.7998 30.8015 24.5321 30.8015 16.7999C30.8015 9.06843 24.534 2.7998 16.8011 2.7998ZM23.2215 22.992C22.9707 23.4032 22.4324 23.5337 22.0211 23.2811C18.734 21.2734 14.5959 20.8186 9.72258 21.9321C9.25298 22.039 8.78486 21.7447 8.67787 21.275C8.57036 20.8052 8.86344 20.337 9.33422 20.2301C14.6673 19.0112 19.2418 19.5362 22.9322 21.7916C23.3435 22.0441 23.4739 22.5807 23.2215 22.992ZM24.9351 19.1794C24.6191 19.6935 23.9471 19.8556 23.4338 19.5396C19.6705 17.226 13.934 16.5562 9.48284 17.9073C8.90557 18.0816 8.29587 17.7563 8.12066 17.18C7.94679 16.6028 8.27229 15.9942 8.84856 15.8187C13.933 14.276 20.254 15.0232 24.5757 17.6789C25.0889 17.9948 25.251 18.6668 24.9351 19.1794ZM25.0822 15.2098C20.57 12.5297 13.1254 12.2833 8.8173 13.5908C8.1255 13.8007 7.39392 13.4101 7.18427 12.7183C6.97463 12.0262 7.36482 11.2951 8.05712 11.0848C13.0025 9.58352 21.2237 9.87357 26.4186 12.9576C27.0422 13.3269 27.2462 14.1306 26.8767 14.7519C26.5089 15.3742 25.7024 15.5793 25.0822 15.2098Z" fill="currentColor" />
    </svg>
  );
}

/** YouTube mark; its mount supplies the active color. */
export function IconSocialYoutube({ size = 34, className }: Props) {
  return (
    <svg width={size} height={size} viewBox="0 0 34 34" fill="none" aria-hidden="true" className={className} xmlns="http://www.w3.org/2000/svg">
      <path fillRule="evenodd" clipRule="evenodd" d="M28.2856 5.61125C29.5514 6.00307 30.5461 7.15334 30.8849 8.61704C31.4971 11.2674 31.4996 16.8004 31.4996 16.8004C31.4996 16.8004 31.4996 22.3335 30.8849 24.9838C30.5461 26.4475 29.5514 27.5978 28.2856 27.9895C25.9938 28.7004 16.7996 28.7004 16.7996 28.7004C16.7996 28.7004 7.60544 28.7004 5.31356 27.9895C4.04784 27.5978 3.05315 26.4475 2.71432 24.9838C2.09961 22.3335 2.09961 16.8004 2.09961 16.8004C2.09961 16.8004 2.09961 11.2674 2.71432 8.61704C3.05315 7.15334 4.04784 6.00307 5.31356 5.61125C7.60544 4.90039 16.7996 4.90039 16.7996 4.90039C16.7996 4.90039 25.9938 4.90039 28.2856 5.61125ZM21.7184 16.8008L13.7166 21.4203V12.1813L21.7184 16.8008Z" fill="currentColor" />
    </svg>
  );
}

/** Apple Podcasts mark; its mount supplies the active color. */
export function IconSocialApplePodcasts({ size = 34, className }: Props) {
  return (
    <svg width={size} height={size} viewBox="0 0 34 34" fill="none" aria-hidden="true" className={className} xmlns="http://www.w3.org/2000/svg">
      <path d="M16.9898 18.4681C17.8704 18.4682 18.6181 18.762 19.0442 19.207C19.2714 19.4342 19.3853 19.6802 19.4232 20.021C19.499 20.6839 19.461 21.2525 19.3758 22.1711C19.3 23.0423 19.1583 24.198 18.9784 25.3817C18.8458 26.224 18.7415 26.6784 18.6468 27.0004C18.4953 27.5212 17.9083 27.9857 16.9898 27.9858C16.0712 27.9858 15.4931 27.5307 15.3321 27.0004C15.2374 26.6784 15.133 26.224 15.0005 25.3817C14.8206 24.2074 14.6789 23.0423 14.6031 22.1711C14.5274 21.2525 14.4799 20.6839 14.5556 20.021C14.5936 19.6802 14.7169 19.4437 14.9347 19.207C15.3608 18.7524 16.1091 18.4681 16.9898 18.4681Z" fill="currentColor" />
      <path d="M16.9615 6.01466C22.4826 6.00519 26.9809 10.494 26.9999 16.0056C26.9999 20.3619 24.206 24.0739 20.3138 25.4376C20.2191 25.4754 20.1149 25.4 20.1338 25.2959C20.1812 24.955 20.2289 24.6138 20.2762 24.2729C20.2858 24.1499 20.3617 24.0461 20.4753 23.9988C23.553 22.6445 25.712 19.5758 25.712 16.0056C25.7119 11.176 21.7631 7.25567 16.924 7.29339C12.1984 7.33127 8.33416 11.1855 8.28681 15.9206C8.249 19.5287 10.4181 22.6446 13.5242 23.9988C13.6281 24.0556 13.7036 24.1501 13.7226 24.2729C13.7604 24.6138 13.8176 24.955 13.865 25.2959C13.8744 25.3906 13.7797 25.4754 13.685 25.4376C9.75493 24.0549 6.94207 20.2859 6.99889 15.8823C7.07478 10.4466 11.5257 6.03364 16.9615 6.01466Z" fill="currentColor" />
      <path d="M16.7333 9.3294C20.5402 9.17789 23.6937 12.2366 23.6844 16.0056C23.6844 18.2973 22.52 20.3336 20.7586 21.5363C20.664 21.6026 20.541 21.5271 20.5504 21.423C20.5693 21.0158 20.5785 20.6559 20.5595 20.2867C20.5595 20.1636 20.6068 20.0495 20.692 19.9643C21.7432 18.97 22.4064 17.5686 22.4064 16.0155C22.4064 12.9662 19.8681 10.485 16.7808 10.6081C13.9399 10.7219 11.6575 13.0322 11.5816 15.8731C11.5438 17.483 12.207 18.9415 13.2961 19.9643C13.3908 20.0495 13.438 20.1635 13.4285 20.2867C13.4096 20.6653 13.4093 21.0251 13.4377 21.4322C13.4377 21.5363 13.3147 21.6031 13.2294 21.5462C11.4396 20.3246 10.2753 18.2594 10.3037 15.9298C10.3512 12.3882 13.1917 9.46215 16.7333 9.3294Z" fill="currentColor" />
      <path d="M16.9891 12.8616C18.2769 12.8616 19.3189 13.9126 19.3191 15.1909C19.3191 16.4788 18.277 17.5209 16.9891 17.5209C15.7013 17.5207 14.6598 16.4787 14.6598 15.1909C14.66 13.9032 15.7014 12.8618 16.9891 12.8616Z" fill="currentColor" />
    </svg>
  );
}

/** Single-tone search mark for compact controls. */
export function IconSearchGlyph({ size = 16, className }: Props) {
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none" aria-hidden="true" className={className} xmlns="http://www.w3.org/2000/svg">
      <path d="M14.3538 13.6462L11.2244 10.5175C12.1314 9.42857 12.5837 8.03187 12.4872 6.61796C12.3906 5.20405 11.7527 3.88178 10.7061 2.92623C9.65949 1.97068 8.28478 1.45541 6.86795 1.48761C5.45111 1.51981 4.10123 2.097 3.09912 3.09911C2.09701 4.10123 1.51981 5.4511 1.48761 6.86794C1.45541 8.28478 1.97069 9.65948 2.92624 10.7061C3.88179 11.7527 5.20405 12.3906 6.61796 12.4871C8.03187 12.5837 9.42857 12.1314 10.5175 11.2244L13.6463 14.3538C13.6927 14.4002 13.7479 14.4371 13.8086 14.4622C13.8693 14.4873 13.9343 14.5003 14 14.5003C14.0657 14.5003 14.1308 14.4873 14.1915 14.4622C14.2522 14.4371 14.3073 14.4002 14.3538 14.3538C14.4002 14.3073 14.4371 14.2521 14.4622 14.1914C14.4873 14.1308 14.5003 14.0657 14.5003 14C14.5003 13.9343 14.4873 13.8692 14.4622 13.8086C14.4371 13.7479 14.4002 13.6927 14.3538 13.6462ZM2.50001 7C2.50001 6.10998 2.76393 5.23995 3.25839 4.49993C3.75286 3.75991 4.45566 3.18314 5.27793 2.84254C6.1002 2.50195 7.005 2.41283 7.87791 2.58647C8.75083 2.7601 9.55265 3.18868 10.182 3.81802C10.8113 4.44736 11.2399 5.24918 11.4135 6.12209C11.5872 6.99501 11.4981 7.89981 11.1575 8.72207C10.8169 9.54434 10.2401 10.2471 9.50007 10.7416C8.76005 11.2361 7.89002 11.5 7.00001 11.5C5.80694 11.4987 4.66311 11.0241 3.81949 10.1805C2.97586 9.33689 2.50133 8.19307 2.50001 7Z" fill="currentColor" />
    </svg>
  );
}
