/** SANDBOX 2 — the engine section remounted with the DISTANCE-MAPPED
 * free scroll island.
 *
 * A byte-for-byte copy of engines.tsx (spec 020 server component) with
 * exactly one change: it mounts EnginesFree2Scroll (ten scroll stops —
 * the b drawings on the scroll path) instead of EnginesScroll (the
 * canonical §9 R20 paged island). Mounted ONLY by /engines-free-2;
 * compare against /engines-free (dwell-timer variant) and
 * /engines-next (canonical paged).
 *
 * The markup keeps the canonical class names, so engines.css styles it
 * with no CSS changes. If this contract is adopted, this file is
...
import { EnginesFree2Scroll } from "./engines-scroll-free-2";
...
export function EnginesFree2Section() {
  return (
    <section
